...
def evaluation(args):
  ...
  # If exp_name is provided, look for checkpoints in results_dir/exp_name/
  checkpoint_dir = os.path.join(args.results_dir, args.exp_name) if args.exp_name else args.results_dir
  worker.run(logdir=checkpoint_dir, eval_checkpoint=args.eval_checkpoint)
...
def get_args_parser():
      ...
      parser.add_argument('--max_steps',type=int,default=int(1e10))
      return parser
...
if __name__ == '__main__':
    ...
    if args.train:
       ...
       # Use full diffusion timesteps for proper reconstruction (1000 for high quality)
       # You can reduce to 100-250 for faster sampling with slight quality loss
       args.num_diffusion_timesteps=1000
