...
def make_optimizer(
  ...
  # CHECK: Verify loss is finite before computing gradients
  ...
  # CRITICAL: Replace NaN/Inf gradients with zeros to prevent weight corruption
  sanitized_grads_and_vars = []
  for g, v in grads_and_vars:
    if g is not None:
      # Replace NaN/Inf with zeros (no per-layer print to avoid log flooding)
      g = tf.where(tf.is_finite(g), g, tf.zeros_like(g))
    sanitized_grads_and_vars.append((g, v))

  # clip gradient
  clipped_grads, gnorm = tf.clip_by_global_norm([g for (g, _) in sanitized_grads_and_vars if g is not None], grad_clip)
  grads_and_vars = [(g, v) for g, (_, v) in zip(clipped_grads, sanitized_grads_and_vars)]
  
  # Only print when gradient norm is effectively zero (all NaN/zero gradients)
  with tf.control_dependencies([tf.cond(
    tf.less(gnorm, 1e-10),
    lambda: tf.print("CRITICAL: Gradient norm =", gnorm, "- All gradients NaN/zero; weights will NOT update!"),
    lambda: tf.no_op()
  )]):
    gnorm = tf.identity(gnorm)
