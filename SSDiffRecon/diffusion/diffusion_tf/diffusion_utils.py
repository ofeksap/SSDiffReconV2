...
class GaussianDiffusion2:
  ...
  def fft2c(self, im):
      # Add epsilon to prevent numerical instability in FFT operations
      im = im + 1e-8
      return tf.signal.fftshift(tf.signal.fft2d(tf.signal.ifftshift(im, axes=[-1,-2])), axes=[-1,-2]) 

  def ifft2c(self, d):
      # Add epsilon to prevent numerical instability in IFFT operations
      d = d + 1e-8
  ...
  def partial_kspace_loss_ixi(self, us_im, loss_mask, generated):
      # Clip inputs to prevent extreme values that could cause NaN in FFT
      us_im = tf.clip_by_value(us_im, -10.0, 10.0)
      generated = tf.clip_by_value(generated, -10.0, 10.0)
