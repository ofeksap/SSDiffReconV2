...
class EvalWorker:
  ...
  def run(self, logdir, seed=0, eval_checkpoint=""):
    ...
    if eval_checkpoint:
      # Look for specific checkpoint pattern: *{eval_checkpoint}.index
      pattern = os.path.join(logdir, "*" + eval_checkpoint + ".index")
    else:
      # Look for all checkpoint files: *.index
      pattern = os.path.join(logdir, "*.index")
    checkpoint_files = natsorted(glob.glob(pattern), reverse=True)
    if len(checkpoint_files) == 0:
      raise RuntimeError("No checkpoint files found matching pattern: {}".format(pattern))
    
    # Use the first (most recent) checkpoint to get variable list
