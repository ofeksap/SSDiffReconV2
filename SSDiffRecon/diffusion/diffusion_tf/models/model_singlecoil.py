...
def DiffModel(    
    ...
    **kwargs,
    ...
    num_layers = 12
    print(f"DEBUG: noisy_sample shape before fix: {noisy_sample.shape}, num_channels: {num_channels}")
    # change here!
    # original lines:
    ...
    # news lines:
    # noisy_sample.set_shape([None, 2, resolution, resolution])
    # us_im.set_shape([None, 2, resolution, resolution])
    ...
    # forcing shpaes
    noisy_sample.set_shape([None, 2, 256, 256])
    us_im.set_shape([None, 2, 256, 256])
    mask.set_shape([None, 1, 256, 256])
    dlatents_in.set_shape([None, 17, 12, 32])

    noisy_sample = noisy_sample[:, :2, :, :]
    us_im = us_im[:, :2, :, :]
    mask = mask[:, :1, :, :] 

    tf.print("DEBUG - Mask Sum (Sampled Points):", tf.reduce_sum(mask), summarize=10)
    tf.print("DEBUG - Mask Max:", tf.reduce_max(mask), summarize=10)
    tf.print("DEBUG - Undersampled Im Max:", tf.reduce_max(us_im), summarize=10)

    tf.print("DEBUG - noisy_sample max:", tf.reduce_max(noisy_sample))
    # print("DEBUG - us_im max:", tf.reduce_max(us_im), output_stream=sys.stdout)

    noisy_sample = tf.clip_by_value(noisy_sample, -10.0, 10.0)
    us_im = tf.clip_by_value(us_im, -10.0, 10.0)

    print(f"DEBUG: noisy_sample shape after fix: {noisy_sample.shape}")
    # end
    ...
    def data_consistency_layer(x, target, mask):
        ...
        tf.print("DEBUG - Masked pixels count:", tf.reduce_sum(tf.cast(masked, tf.float32)))
    ...
    imgs_out = tf.clip_by_value(imgs_out, -10.0, 10.0)
