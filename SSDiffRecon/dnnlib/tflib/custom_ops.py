...
def _get_cuda_gpu_arch_string():
    ...
    arch_str = "sm_%s%s" % (major, minor)
    
    # CUDA 10.2 only supports up to sm_75 (Turing architecture)
    # For newer architectures (sm_80+), fall back to sm_75 for compatibility
    major_int = int(major)
    if major_int >= 8:
        # Ampere (sm_86) and newer architectures not supported by CUDA 10.2
        # Use sm_75 as fallback (highest supported by CUDA 10.2)
        return "sm_75"
